        ...
        <div class="p-6 bg-white border-b border-gray-200">
          <div class="md:flex md:justify-around">
            ...
            <div class="md:w-1/2 ml-4">
              <h2 class="mb-4text-sm title-font text-gray-500 tracking-widest">{{ $product->category->name }}</h2>
              <div class="flex items-center mb-4">
                <h1 class="mr-4 text-gray-900 text-3xl title-font font-medium">{{ $product->name }}</h1>
                @if (!$isLike)
                  <form method="post" action="{{ route('user.like.add', ['item' => $product->id]) }}">
                    @csrf
                    <button>
                ...
                @else
                  <form method="post" action="{{ route('user.like.delete', ['item' => $product->id]) }}">
                    @csrf
                    <button>
